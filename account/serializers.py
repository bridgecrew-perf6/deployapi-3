...
class RegistrationSerializer(serializers.ModelSerializer):
    password = serializers.CharField(min_length=8, required=True, write_only=True) #сериaлизует когда мы отправляем пост запрос
    password_confirm = serializers.CharField(min_length=8, required=True, write_only=True)
    class Meta:
        model = User
        fields = ('email', 'password', 'password_confirm')

#attrs -> все данные которые отправляет пользователь то же самое что и data
